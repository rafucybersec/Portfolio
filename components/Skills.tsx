import React from 'react';

const SkillImage = ({ src, alt, className }: { src: string; alt: string; className: string }) => (
  <img
    src={src}
    alt={alt}
    className={className}
    loading="lazy"
    decoding="async"
  />
);

// Official SVG Icons for Brands - Using local official brand icons
const Icons = {
  Wazuh: () => (
    <SkillImage
      src="/icons/wazuh.svg"
      alt="Wazuh"
      className="w-12 h-12 sm:w-40 sm:h-40 object-contain"
    />
  ),
  QRadar: () => (
    <SkillImage
      src="/icons/qradar.png"
      alt="IBM QRadar"
      className="w-12 h-12 sm:w-40 sm:h-40 object-contain"
    />
  ),
  ThreatHawk: () => (
    <SkillImage
      src="/icons/threat-hawk.png"
      alt="Threat Hawk"
      className="w-12 h-12 sm:w-40 sm:h-40 object-contain"
    />
  ),
  Threat: () => (
    <SkillImage
      src="/icons/threat.svg"
      alt="Threat Intelligence"
      className="w-8 h-8 sm:w-12 sm:h-12 object-contain"
    />
  ),
  IOCs: () => (
    <SkillImage
      src="/icons/iocs.png"
      alt="IOCs"
      className="w-8 h-8 sm:w-12 sm:h-12 object-contain"
    />
  ),
  Hunting: () => (
    <SkillImage
      src="/icons/hunting.png"
      alt="Hunting"
      className="w-8 h-8 sm:w-12 sm:h-12 object-contain"
    />
  ),
  Burp: () => (
    <SkillImage
      src="/icons/burp.png"
      alt="Burp Suite"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Metasploit: () => (
    <SkillImage
      src="/icons/Metasploit.svg"
      alt="Metasploit"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Python: () => (
    <SkillImage
      src="/icons/python.png"
      alt="Python"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Kali: () => (
    <SkillImage
      src="/icons/kali_linux.svg"
      alt="Kali Linux"
      className="w-8 h-8 sm:w-12 sm:h-12 object-contain"
    />
  ),
  Windows: () => (
    <SkillImage
      src="/icons/windows.svg"
      alt="Windows"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Linux: () => (
    <SkillImage
      src="/icons/linux.svg"
      alt="Linux"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Azure: () => (
    <SkillImage
      src="/icons/microsoft_azure.svg"
      alt="Azure"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Mitre: () => (
    <SkillImage
      src="/icons/mitre.png"
      alt="MITRE ATT&CK"
      className="w-8 h-8 sm:w-40 sm:h-40 object-contain"
    />
  ),
  PowerShell: () => (
    <SkillImage
      src="/icons/powershell.svg"
      alt="PowerShell"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Bash: () => (
    <SkillImage
      src="/icons/bash.svg"
      alt="Bash"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Docker: () => (
    <SkillImage
      src="/icons/docker.svg"
      alt="Docker"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  GitHub: () => (
    <SkillImage
      src="/icons/github.svg"
      alt="GitHub"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  NodeJS: () => (
    <SkillImage
      src="/icons/nodejs.svg"
      alt="Node.js"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  OSINT: () => (
    <SkillImage
      src="/icons/osint.png"
      alt="OSINT"
      className="w-8 h-8 sm:w-40 sm:h-40 object-contain"
    />
  ),
  AzureSentinel: () => (
    <SkillImage
      src="/icons/azure_sentinel.svg"
      alt="Microsoft Sentinel"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  Nmap: () => (
    <SkillImage
      src="/icons/nmap_logo.png"
      alt="Nmap"
      className="w-8 h-8 sm:w-12 sm:h-12 object-contain"
    />
  ),
  ActiveDirectory: () => (
    <SkillImage
      src="/icons/active_directory.png"
      alt="Active Directory"
      className="w-8 h-8 sm:w-40 sm:h-40 object-contain"
    />
  ),
  VMware: () => (
    <SkillImage
      src="/icons/vmware_workstation.svg"
      alt="VMware"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
  ISO27001: () => (
    <SkillImage
      src="/icons/iso_27001.png"
      alt="ISO 27001"
      className="w-8 h-8 sm:w-13 sm:h-13 object-contain"
    />
  ),
  NCAECC: () => (
    <SkillImage
      src="/icons/NCA_ECC.png"
      alt="NCA-ECC"
      className="w-8 h-8 sm:w-40 sm:h-40 object-contain brightness-1000"
    />
  ),
  SAMA: () => (
    <SkillImage
      src="/icons/SAMA_CSF.png"
      alt="SAMA CSF"
      className="w-8 h-8 sm:w-17 sm:h-17 object-contain brightness-1000"
    />
  ),
  ADHICS: () => (
    <SkillImage
      src="/icons/adhics.png"
      alt="ADHICS"
      className="w-8 h-8 sm:w-40 sm:h-40 object-contain brightness-1000"
    />
  ),
  IncidentResponse: () => (
    <SkillImage
      src="/icons/incident_response.png"
      alt="Incident Response"
      className="w-8 h-8 sm:w-13 sm:h-13 object-contain invert"
    />
  ),
  AlertTuning: () => (
    <SkillImage
      src="/icons/alert_tuning.png"
      alt="Alert Tuning"
      className="w-8 h-8 sm:w-12 sm:h-12 object-contain invert"
    />
  ),
  APIIntegration: () => (
    <SkillImage
      src="/icons/api_integration.png"
      alt="API Integration"
      className="w-8 h-8 sm:w-15 sm:h-15 object-contain brightness-100 invert"
    />
  ),
  RuleParsing: () => (
    <SkillImage
      src="/icons/rules_parsing.png"
      alt="Rule Parsing"
      className="w-8 h-8 sm:w-15 sm:h-15 object-contain brightness-100 invert"
    />
  ),
  Vercel: () => (
    <SkillImage
      src="/icons/vercel.svg"
      alt="Vercel"
      className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
    />
  ),
};

import { ShieldAlert, Zap, Cloud, Scale, Bot, AlertTriangle } from 'lucide-react';

const SkillBadge = ({ icon: Icon, name, isCustom = false }: { icon: any, name: string, isCustom?: boolean }) => (
  <div className="group relative flex items-center justify-center p-0 hover:border-cyber-green transition-all duration-500 w-16 h-16 sm:w-18 sm:h-18 shadow-none hover:shadow-[0_0_25px_rgba(0,255,157,0.5)] cursor-pointer">

    {/* Icon container */}
    <div className="text-white dark:text-white group-hover:text-cyber-green-dark dark:group-hover:text-cyber-green transition-all duration-300">
      {isCustom ? <Icon /> : <Icon className="w-8 h-8 sm:w-10 sm:h-10 transition-transform duration-300" strokeWidth={1.5} />}
    </div>

    {/* Tooltip with slide-up animation */}
    <span className="absolute -bottom-7 left-1/2 transform -translate-x-1/2 translate-y-2 group-hover:translate-y-0 px-1 py-1 bg-cyber-green-dark dark:bg-cyber-green text-white dark:text-black text-xs font-bold rounded opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap pointer-events-none z-20 font-satoshi">
      {name}
    </span>
  </div>
);

const Skills: React.FC = () => {
  return (
    <section id="skills" className="py-20 transition-colors duration-300 scroll-mt-20 relative overflow-hidden">
      {/* Video Background for Skills - GREEN Filter */}
      <div 
        className="absolute inset-0 w-full h-full z-0 flex items-center justify-center overflow-hidden pointer-events-none"
        style={{
          maskImage: 'linear-gradient(to bottom, transparent, black 15%, black 85%, transparent)',
          WebkitMaskImage: 'linear-gradient(to bottom, transparent, black 15%, black 85%, transparent)'
        }}
      >
        <video
          className="absolute min-w-full min-h-full object-cover opacity-30"
          preload="none"
          playsInline
          loop
          muted
          autoPlay
          aria-hidden="true"
          style={{ filter: 'sepia(100%) hue-rotate(70deg) saturate(2) brightness(0.5)' }}
        >
          <source src="/videos/skills-bg.webm" type="video/webm" />
        </video>
      </div>

      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-10 relative">
          <h2 className="text-3xl md:text-5xl font-bold text-white dark:text-white relative z-10 font-satoshi">
            Technical <span className="text-cyber-green-dark dark:text-cyber-green">Skills</span>
          </h2>
        </div>

        <div className="max-w-6xl mx-auto space-y-4">

          {/* Row 1: SIEM & SOC */}
          <div className="flex flex-col md:flex-row gap-4 items-start border-b border-white/5 pb-3">
            <div className="w-full md:w-64 flex-shrink-0">
              <h3 className="text-xl font-bold text-cyber-green-dark dark:text-cyber-green font-satoshi flex items-center gap-2">
                <ShieldAlert className="text-cyber-green-dark dark:text-cyber-green w-6 h-6" />
                SIEM & SOC
              </h3>
              <p className="text-sm text-white dark:text-white mt-2 font-satoshi">Threat Detection & Response</p>
            </div>
            <div className="flex flex-wrap gap-4 flex-1">
              <SkillBadge icon={Icons.Wazuh} name="Wazuh" isCustom={true} />
              <SkillBadge icon={Icons.QRadar} name="IBM QRadar" isCustom={true} />
              <SkillBadge icon={Icons.ThreatHawk} name="Threat Hawk" isCustom={true} />
              <SkillBadge icon={Icons.AzureSentinel} name="Microsoft Sentinel" isCustom={true} />
              <SkillBadge icon={Icons.Threat} name="Threat Intelligence" isCustom={true} />
              <SkillBadge icon={Icons.IOCs} name="IOC Analysis" isCustom={true} />
              <SkillBadge icon={Icons.Hunting} name="Threat Hunting" isCustom={true} />
            </div>
          </div>

          {/* Row 2: Offensive Security */}
          <div className="flex flex-col md:flex-row gap-4 items-start border-b border-white/5 pb-3">
            <div className="w-full md:w-64 flex-shrink-0">
              <h3 className="text-xl font-bold text-cyber-green-dark dark:text-cyber-green font-satoshi flex items-center gap-2">
                <Zap className="text-cyber-blue-dark dark:text-cyber-blue w-6 h-6" />
                Offensive Security
              </h3>
              <p className="text-sm text-white dark:text-white mt-2 font-satoshi">VAPT & Exploitation</p>
            </div>
            <div className="flex flex-wrap gap-4 flex-1">
              <SkillBadge icon={Icons.Kali} name="Kali Linux" isCustom={true} />
              <SkillBadge icon={Icons.Burp} name="Burp Suite" isCustom={true} />
              <SkillBadge icon={Icons.Metasploit} name="Metasploit" isCustom={true} />
              <SkillBadge icon={Icons.Mitre} name="MITRE ATT&CK" isCustom={true} />
              <SkillBadge icon={Icons.OSINT} name="OSINT" isCustom={true} />
              <SkillBadge icon={Icons.Nmap} name="Nmap" isCustom={true} />
            </div>
          </div>

          {/* Row 3: OS & Cloud */}
          <div className="flex flex-col md:flex-row gap-4 items-start border-b border-white/5 pb-3">
            <div className="w-full md:w-64 flex-shrink-0">
              <h3 className="text-xl font-bold text-cyber-green-dark dark:text-cyber-green font-satoshi flex items-center gap-2">
                <Cloud className="text-cyber-purple w-6 h-6" />
                OS & Cloud
              </h3>
              <p className="text-sm text-white dark:text-white mt-2 font-satoshi">Infrastructure & Hardening</p>
            </div>
            <div className="flex flex-wrap gap-4 flex-1">
              <SkillBadge icon={Icons.Windows} name="Windows Security" isCustom={true} />
              <SkillBadge icon={Icons.Linux} name="Linux Hardening" isCustom={true} />
              <SkillBadge icon={Icons.Azure} name="Azure Cloud" isCustom={true} />
              <SkillBadge icon={Icons.ActiveDirectory} name="Active Directory" isCustom={true} />
              <SkillBadge icon={Icons.VMware} name="VMware" isCustom={true} />
            </div>
          </div>

          {/* Row 4: Governance */}
          <div className="flex flex-col md:flex-row gap-4 items-start border-b border-white/5 pb-3">
            <div className="w-full md:w-64 flex-shrink-0">
              <h3 className="text-xl font-bold text-cyber-green-dark dark:text-cyber-green font-satoshi flex items-center gap-2">
                <Scale className="text-yellow-600 dark:text-yellow-500 w-6 h-6" />
                Governance
              </h3>
              <p className="text-sm text-white dark:text-white mt-2 font-satoshi">GRC & Standards</p>
            </div>
            <div className="flex flex-wrap gap-4 flex-1">
              <SkillBadge icon={Icons.ISO27001} name="ISO 27001" isCustom={true} />
              <SkillBadge icon={Icons.NCAECC} name="NCA-ECC" isCustom={true} />
              <SkillBadge icon={Icons.SAMA} name="SAMA" isCustom={true} />
              <SkillBadge icon={Icons.ADHICS} name="ADHICS" isCustom={true} />
            </div>
          </div>

          {/* Row 5: Security Operations */}
          <div className="flex flex-col md:flex-row gap-4 items-start border-b border-white/5 pb-3">
            <div className="w-full md:w-64 flex-shrink-0">
              <h3 className="text-xl font-bold text-cyber-green-dark dark:text-cyber-green font-satoshi flex items-center gap-2">
                <AlertTriangle className="text-orange-600 dark:text-orange-500 w-6 h-6" />
                Security Ops
              </h3>
              <p className="text-sm text-white dark:text-white mt-2 font-satoshi">IR & Engineering</p>
            </div>
            <div className="flex flex-wrap gap-4 flex-1">
              <SkillBadge icon={Icons.IncidentResponse} name="Incident Response" isCustom={true} />
              <SkillBadge icon={Icons.IncidentResponse} name="Incident Documentation" isCustom={true} />
              <SkillBadge icon={Icons.RuleParsing} name="Rule Parsing" isCustom={true} />
              <SkillBadge icon={Icons.AlertTuning} name="Alert Tuning" isCustom={true} />
              <SkillBadge icon={Icons.APIIntegration} name="API Integration" isCustom={true} />
            </div>
          </div>

          {/* Row 5: Automation */}
          <div className="flex flex-col md:flex-row gap-4 items-start border-b border-white/5 pb-3">
            <div className="w-full md:w-64 flex-shrink-0">
              <h3 className="text-xl font-bold text-cyber-green-dark dark:text-cyber-green font-satoshi flex items-center gap-2">
                <Bot className="text-red-600 dark:text-red-500 w-6 h-6" />
                Automation
              </h3>
              <p className="text-sm text-white dark:text-white mt-2 font-satoshi">Scripting & DevOps</p>
            </div>
            <div className="flex flex-wrap gap-4 flex-1">
              <SkillBadge icon={Icons.Python} name="Python" isCustom={true} />
              <SkillBadge icon={Icons.Bash} name="Bash" isCustom={true}/>
              <SkillBadge icon={Icons.PowerShell} name="PowerShell" isCustom={true}/>
              <SkillBadge icon={Icons.Docker} name="Docker" isCustom={true} />
              <SkillBadge icon={Icons.GitHub} name="GitHub" isCustom={true} />
              <SkillBadge icon={Icons.NodeJS} name="Node.js" isCustom={true} />
              <SkillBadge icon={Icons.Vercel} name="Vercel" isCustom={true} />
            </div>
          </div>

        </div>
      </div>
    </section>
  );
};

export default Skills;
