...
const Experience: React.FC = () => {
  return (
    <section id="experience" className="py-20 bg-cyber-dark scroll-mt-20">
          ...
          <div className="space-y-12">
            {experienceData.map((item, index) => (
              <div key={item.id} className={`relative flex flex-col md:flex-row gap-8 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                {/* Dot */}
                <div className="absolute left-0 md:left-1/2 w-4 h-4 bg-cyber-black border-4 border-cyber-green rounded-full transform -translate-x-[7px] md:-translate-x-1/2 mt-6 z-10"></div>

                {/* Content Side */}
                <div className="flex-1 ml-8 md:ml-0">
                  <div className="bg-cyber-gray p-6 rounded-xl border border-white/5 transition-all duration-300 hover:scale-[1.02] hover:border-cyber-green hover:shadow-[0_0_20px_rgba(0,255,136,0.15)] group">
                    ...
                    <ul className="space-y-3">
                      {item.points.map((point, i) => (
                        <li key={i} className="text-cyber-text/80 text-sm flex items-start gap-2">
