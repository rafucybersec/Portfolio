    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${
      ...
      <div className="container mx-auto px-4 flex justify-between items-center">
        <a href="#" className="flex items-center gap-2 group">
          ...
          <span className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Rafay<span className="text-cyber-green">.Sec</span></span>
        </a>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="relative text-sm font-medium text-gray-600 dark:text-cyber-text hover:text-cyber-green dark:hover:text-cyber-green transition-colors duration-300 before:absolute before:-bottom-1 before:left-0 before:w-0 before:h-0.5 before:bg-cyber-green before:transition-all before:duration-300 hover:before:w-full"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="flex flex-col p-4 gap-4">
          {navLinks.map((link) => (
            <a
              ...
              className="text-gray-700 dark:text-cyber-text hover:text-cyber-green font-medium py-2 px-4 hover:bg-gray-100 dark:hover:bg-white/5 rounded-lg transition-colors"
            >
              {link.name}
            </a>
          ))}
